...
python3 preprocess.py --dataset=cosql --remove_from > ${LOGDIR}.data.log 2>&1 &
wait
...
wait
cp run_cosql_editsql.sh ./${LOGDIR}/
wait
cp ./model/schema_interaction_model.py ./${LOGDIR}/
wait

CUDA_VISIBLE_DEVICES=$devices python3 run.py --raw_train_filename="../data/cosql_data_removefrom/train.pkl" \
...
python3 postprocess_eval.py --dataset=cosql --split=dev --pred_file $LOGDIR/valid_use_predicted_queries_predictions.json --remove_from > ${LOGDIR}.eval.log 2>&1 &
