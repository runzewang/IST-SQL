...
class SequencePredictorWithSchema(torch.nn.Module):
    ...
    def get_output_token_embedding(self, output_token, input_schema, vocabulary_emb, snippets):
    ...
    def forward(self,
                ...
                previous_schema_memory=None,
                select_schema_memory=None,
                input_schema=None,
                dropout_amount=0.):
        ...
        while continue_generating:
            if len(sequence) == 0 or sequence[-1] != EOS_TOK:
                ...
                if gold_sequence:
                    ...
                    output_token_embedding = self.get_output_token_embedding(output_token, input_schema,vocabulary_emb, snippets)
                    decoder_input = self.get_decoder_input(output_token_embedding, prediction)
                    sequence.append(gold_sequence[index])
                    if output_token in input_schema.column_names_surface_form_to_id:
                        select_schema_memory[input_schema.column_names_surface_form_to_id[output_token]] = 1.
                    ...
                    # print('distribution_map', distribution_map)
                    ...
                    output_token_embedding = self.get_output_token_embedding(argmax_token, input_schema,vocabulary_emb, snippets)

                    decoder_input = self.get_decoder_input(output_token_embedding, prediction)
                    if argmax_token in input_schema.column_names_surface_form_to_id:
                        select_schema_memory[input_schema.column_names_surface_form_to_id[argmax_token]] = 1.
