...
class MulRel(torch.nn.Module):
    ...
    def __init__(self, rel_num, dim_size, use_emb_w, src_none_node=False, tar_none_node=False, softmax_src=True, \
            tar_sum=True, use_tar_node=False, diagonal_matrix=False, use_normalize=False, score_use_normalize=False, dropout_mount=0.):
        '''
    ...
    def forward(self, src_emb, tar_emb, src_mask, tar_mask, dropout_amount=0.):
        '''
        ...
        if self.src_none_node is not None:
            ...
            src_emb_ = F.dropout(src_emb_, p=dropout_amount, training=self.training)
        else:
            src_mask_ = src_mask
            src_emb_ = F.dropout(src_emb, p=dropout_amount, training=self.training)
        ...
        if self.tar_none_node is not None:
            ...
            tar_emb_ = F.dropout(tar_emb_, p=dropout_amount, training=self.training)
        else:
            tar_mask_ = tar_mask
            tar_emb_ = F.dropout(tar_emb, p=dropout_amount, training=self.training)
        ...
        src_tar_score = src_tar_score.add((1-src_tar_mask).mul(-1e10).unsqueeze(0))
        ...
        if self.tar_sum:
            ...
            if self.src_none_node is not None:
                mul_emb = mul_emb[:, :-1, :, :]
        mul_emb = F.dropout(mul_emb, p=dropout_amount, training=self.training)
        return mul_emb
