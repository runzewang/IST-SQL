...
class PredictionInputWithSchema(namedtuple('PredictionInputWithSchema',
                                 ('decoder_state',
                                  ...
                                  'previous_schema_memory',
                                  'select_schema_memory',
                                  'input_schema'))):
    """ Inputs to the token predictor. """
    __slots__ = ()
...
class TokenPredictor(torch.nn.Module):
    ...
    def _score_vocabulary_tokens(self, state, vocabulary_emb):
        scores = torch.t(torch_utils.linear_layer(state, vocabulary_emb.permute(1, 0), self.vocabulary_biases))
...
class SchemaTokenPredictor(TokenPredictor):
    ...
    def __init__(self, params, vocabulary, utterance_attention_key_size, schema_attention_key_size, snippet_size):
        ...
        elif params.use_query_attention:
            self.state_transform_weights = torch_utils.add_params((params.decoder_state_size + utterance_attention_key_size + params.encoder_state_size, params.decoder_state_size), "weights-state-transform")
        ...
        if self.params.use_copy_switch:
            if self.params.use_schema_attention:
                self.state2copyswitch_transform_weights = torch_utils.add_params((params.decoder_state_size+schema_attention_key_size+utterance_attention_key_size*2, 1), "weights-state-transform")
            else:
                self.state2copyswitch_transform_weights = torch_utils.add_params((params.decoder_state_size+utterance_attention_key_size*2, 1), "weights-state-transform")
                
        # self.state2copyswitch_transform_weights_2 = torch_utils.add_params((params.decoder_state_size + utterance_attention_key_size + params.encoder_state_size, 1), "weights-state-transform")
    ...
    # def _get_copy_switch_2(self, state):
    #     copy_switch = torch.sigmoid(torch_utils.linear_layer(state, self.state2copyswitch_transform_weights_2))
    #     return copy_switch.squeeze()
    ...
    def forward(self, prediction_input, dropout_amount=0.):
        ...
        vocabulary_emb = prediction_input.vocabulary_emb

        previous_schema_memory=torch.tensor(prediction_input.previous_schema_memory).cuda()
        select_schema_memory=torch.tensor(prediction_input.select_schema_memory).cuda()
        ...
        elif self.params.use_query_attention:
            state_and_attn = torch.cat([decoder_state, utterance_attention_results.vector, query_attention_results.vector], dim=0)
        else:
            state_and_attn = torch.cat([decoder_state, utterance_attention_results.vector], dim=0)

        intermediate_state = self._get_intermediate_state(state_and_attn, dropout_amount=dropout_amount)
        vocab_scores, vocab_tokens = self._score_vocabulary_tokens(intermediate_state, vocabulary_emb)
        ...
        # print('schema_scores', schema_scores.size())
        # copy_switch_2 = self._get_copy_switch_2(state_and_attn)
        # schema_scores = schema_scores.squeeze(1) * select_schema_memory * copy_switch_2 + \
        #     schema_scores.squeeze(1) * (1-select_schema_memory) * (1 - copy_switch_2)
        # print('schema_scores', schema_scores.size())
        # print('final_scores', final_scores.size())
