...
import torch
from occumpy_memory import occumpy_mem, get_host_name, setup_seed
# np.random.seed(0)
# random.seed(0)
...
def train(model, data, params):
    ...
    # print('num_train_original', num_train_original)
    ...
    #     for itme in batch.items:
    #         aaa.append(len(itme))
    # bb = sum(aaa)
    # log.put(
    #     "Actual number of used training examples 22222222:\t" +
    #     str(bb))
    ...
    while keep_training:
        ...
        if not params.scheduler and not params.clip_lr:
        ...
        saved = False
        if params.clip_lr:
            if string_accuracy > 54 and not lr_changed:
                model.set_learning_rate(0.1 * params.initial_learning_rate)
                lr_changed = True
        ...
        if not saved and string_accuracy > maximum_string_accuracy:
            ...
            last_save_file = os.path.join(params.logdir, "save_best")
            last_two_save_file = os.path.join(params.logdir, "save_best_two")
            if os.path.exists(last_save_file):
                os.rename(last_save_file, last_two_save_file)
...
def evaluate(model, data, params, last_save_file, split):
    ...
    if params.interaction_level or params.use_predicted_queries:
        ...
        if params.interaction_level:
            eval_result = evaluate_interaction_sample(
                examples,
                model,
                name=full_name,
            ...
            # eval_result = evaluate_interaction_sample(examples,
            #             model,
            #             params.eval_maximum_sql_length,
            #             name=full_name,
            #             write_results=True,
...
def main():
    ...
    model.build_optim()
    model, [model.trainer, model.bert_trainer] = amp.initialize(model, [model.trainer, model.bert_trainer], opt_level="O1")
